...
# The input/output directory for the video file
...
def get_video_duration(video_path: Path) -> float:
    ...
    try:
        if not video_path.exists() or video_path.stat().st_size == 0:
            print(f"File does not exist or is empty: {video_path.name}")
            return 0.0

        probe = ffmpeg.probe(str(video_path))
        
        # Prioritize video stream duration
...
def add_background_music(
    ...
    final_output_path: Path # This is the target path (same as input_video_path in your pipeline)
):
    """
    Adds background music to a video file, using a temporary file for mixing
    and then replaces the original video file.
    """
    setup_directories() 
    ...
    if not music_path.exists():
        ...
        # We explicitly skip the copy if paths are the same to avoid SameFileError
        if video_path != final_output_path:
            try:
                shutil.copy2(str(video_path), str(final_output_path))
    ...
    try:
        ...
        if video_duration <= 0:
            print("Error: Could not determine video duration. Skipping music addition.")
            # Fallback (same as above): ensure the original file remains for the next step
            if video_path != final_output_path:
                try:
                    shutil.copy2(str(video_path), str(final_output_path))
                except shutil.SameFileError:
        ...
        # 4. Process background music (volume and trim)
        ...
        (
            ...
            .overwrite_output()
            # Set log level to 'error' to suppress most FFmpeg warnings but still show errors
            .run(quiet=True) 
        )
        ...
        # Ensure the temp file is deleted if it failed
        if temp_output_path.exists():
            os.remove(temp_output_path)
            
        # Fallback: ensure original file is present for next step if mix failed
        if video_path != final_output_path:
            try:
                shutil.copy2(str(video_path), str(final_output_path))
            except shutil.SameFileError:
